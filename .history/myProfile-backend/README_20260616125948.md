...
Test it's alive by visiting `http://localhost:3000/api/health` in your browser.

## 4. Connect the frontend

In `myProfile/js/features/contact-api.js`, the `API_BASE_URL` is already set to `http://localhost:3000` for local development. When you deploy the backend (Render, Railway, etc.), update that constant to your live backend URL.
